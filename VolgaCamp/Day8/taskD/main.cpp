...
{
    int p, q;
    fra(int p1, int q1){p=p1;q=q1;}
    bool operator<(const fra& a)
    {
        return p * a.q < a.p*q;
    }
    bool operator>=(const fra& a)
    ...
    {
        return fra(p*a.q, q*a.p);
    }
};
int main()
{
    int n, sum=0, a, m, k, b, c, d, p, r, i, s;
    ifstream fin("cut-potatoes.in");
    ofstream fout("cut-potatoes.out");
    fra BestMin(1, 100), BestMax(101, 1), currentMin(1,1), currentMax(1,1);
    fin >> n >> k;
    vi A(n);
    std::vector<std::vector<fra>> B(n, vector<fra>(k, fra(1, 1)));
    fin >> A;
    forx(i, n) forx(j, k) B[i][j]=fra(A[i], k-j);
    forx(i, n) forx(j, k)
        {
            currentMin = fra(A[i], k-j);
            currentMax = currentMin;
            for(auto &v:B) if(lower_bound(all(v), currentMin)!=v.end()) currentMax=max(currentMax, *lower_bound(all(v), currentMin));
    ...
    forx(i, n)
    {
        forx(j, k) if(fra(A[i], k-j)<=BestMax&&fra(A[i], k-j)>=BestMin)
            {
                fout << k-j << " ";
                break;
            }
